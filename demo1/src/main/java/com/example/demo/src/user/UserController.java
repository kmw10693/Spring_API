...
public class UserController {
    ...
    public BaseResponse<PostUserRes> createUser(@RequestBody PostUserReq postUserReq) {
        ...
        // 패스워드에 값이 존재하는지, 빈 값으로 요청하지는 않았는지 검사합니다.
        if(postUserReq.getPassword() == null){
            return new BaseResponse<>(POST_USERS_INVALID_PASSWORD);
        }
        // 주소에 값이 존재하는지, 빈 값으로 요청하지는 않았는지 검사합니다.
        if(postUserReq.getAddress() == null){
            return new BaseResponse<>(POST_USERS_INVALID_ADDRESS);
        }
        // 이름에 값이 존재하는지, 빈 값으로 요청하지는 않았는지 검사합니다.
 ...
 */
    ...
    public BaseResponse<GetPointRes> getUser3(@PathVariable("userIdx") int userIdx) {
        // @PathVariable RESTful(URL)에서 명시된 파라미터({})를 받는 어노테이션, 이 경우 userId값을 받아옴.
        //  null값 or 공백값이 들어가는 경우는 적용하지 말 것
        //  .(dot)이 포함된 경우, .을 포함한 그 뒤가 잘려서 들어감
        // Get Users
        try {
            GetPointRes getUserRes = userProvider.getPoint(userIdx);
            return new BaseResponse<>(getUserRes);
        } catch (BaseException exception) {
            return new BaseResponse<>((exception.getStatus()));
        }
    ...
    @ResponseBody   // return되는 자바 객체를 JSON으로 바꿔서 HTTP body에 담는 어노테이션.
    //  JSON은 HTTP 통신 시, 데이터를 주고받을 때 많이 쓰이는 데이터 포맷.
    @GetMapping("/points") // (GET) 127.0.0.1:9000/app/users
    // GET 방식의 요청을 매핑하기 위한 어노테이션
    public BaseResponse<List<GetPointRes>> getPoints(@RequestParam(required = false) String usePlace) {
        //  @RequestParam은, 1개의 HTTP Request 파라미터를 받을 수 있는 어노테이션(?뒤의 값). default로 RequestParam은 반드시 값이 존재해야 하도록 설정되어 있지만, (전송 안되면 400 Error 유발)
        //  지금 예시와 같이 required 설정으로 필수 값에서 제외 시킬 수 있음
        //  defaultValue를 통해, 기본값(파라미터가 없는 경우, 해당 파라미터의 기본값 설정)을 지정할 수 있음
        try {
            if (usePlace == null) { // query string인 nickname이 없을 경우, 그냥 전체 유저정보를 불러온다.
                List<GetPointRes> getPointsRes = userProvider.getPoints();
                return new BaseResponse<>(getPointsRes);
            }
            // query string인 nickname이 있을 경우, 조건을 만족하는 유저정보들을 불러온다.
            List<GetPointRes> getUsersRes = userProvider.getPointsByNickname(usePlace);
            return new BaseResponse<>(getUsersRes);
        } catch (BaseException exception) {
            return new BaseResponse<>((exception.getStatus()));
        }
