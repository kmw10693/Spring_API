...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (name, password,phoneNum, email,address) VALUES (?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getName(),postUserReq.getPassword(), postUserReq.getPhoneNum(), postUserReq.getEmail(),postUserReq.getAddress()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, password,email,name,address,status from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    // 로그인: 해당 email에 해당하는 user의 status를 불러온다.
    ...
    public GetCouponRes getCoupon(int userIdx) {
        ...
        return this.jdbcTemplate.queryForObject(getUserQuery,
                (rs, rowNum) -> new GetCouponRes(
                        ...
                        rs.getInt("couponValidate"),
                        rs.getString("status")),
                getUserParams); // 한 개의 회원정보를 얻기 위한 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
    }
    // 쿠폰정보 변경
    public int modifyCouponName(PatchCouponReq patchCouponReq) {
        String modifyUserNameQuery = "update Coupon set couponInfo = ? where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{patchCouponReq.getCouponInfo(), patchCouponReq.getUserIdx()}; // 주입될 값들(nickname, userIdx) 순

        return this.jdbcTemplate.update(modifyUserNameQuery, modifyUserNameParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }

    // 해당 UserIdx를 갖는 포인트 정보 조회
    public GetPointRes getPoint(int userIdx) {
        String getUserQuery = "select * from Point where userIdx = ?"; // 해당 userIdx를 만족하는 유저를 조회하는 쿼리문
        int getUserParams = userIdx;
        return this.jdbcTemplate.queryForObject(getUserQuery,
                (rs, rowNum) -> new GetPointRes(
                        rs.getInt("userIdx"),
                        rs.getInt("point"),
                        rs.getInt("deletePoint"),
                        rs.getInt("useDate"),
    ...
    public List<GetPointRes> getPointsByNickname(String usePlace) {
        String getUsersByNicknameQuery = "select * from Point where usePlace =?"; // 해당 이메일을 만족하는 유저를 조회하는 쿼리문
        String getUsersByNicknameParams = usePlace;
        return this.jdbcTemplate.query(getUsersByNicknameQuery,
                (rs, rowNum) -> new GetPointRes(
                        rs.getInt("userIdx"),
                        rs.getInt("point"),
                        ...
                        rs.getInt("useDate"),
                        rs.getString("usePlace"),
                        rs.getInt("pointSave")), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
                getUsersByNicknameParams); // 해당 닉네임을 갖는 모든 User 정보를 얻기 위해 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
    }
    // 회원정보 변경
    public int modifyPointName(PatchPointReq patchPointReq) {
        String modifyUserNameQuery = "update Point set point = ? where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{patchPointReq.getPoint(), patchPointReq.getUserIdx()}; // 주입될 값들(nickname, userIdx) 순
