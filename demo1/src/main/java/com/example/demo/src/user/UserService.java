...
//  *********************************************************************
    ...
    // 포인트정보 수정(Patch)
    public void modifyPointName(PatchPointReq patchPointReq) throws BaseException {
        try {
            int result = userDao.modifyPointName(patchPointReq); // 해당 과정이 무사히 수행되면 True(1), 그렇지 않으면 False(0)입니다.
            if (result == 0) { // result값이 0이면 과정이 실패한 것이므로 에러 메서지를 보냅니다.
            }
        } catch (Exception exception) { // DB에 이상이 있는 경우 에러 메시지를 보냅니다.
        }
